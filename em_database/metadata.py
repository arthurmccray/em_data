...
``datasets/json-schema.json`` closes the dataset entry with
...
SCHEMA_PATH = DATASETS_DIR / "json-schema.json"
VENDORS_PATH = DATASETS_DIR / "vendors.yaml"

# Files in datasets/ that are not dataset collections.
...
def dataset_files() -> list[Path]:
    ...
    return sorted(p for p in DATASETS_DIR.rglob("*.y*ml") if p.name not in NON_DATASET_FILES)
