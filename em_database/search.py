...
def filter(**criteria: Any) -> list[DownloadableDataset]:  # noqa: A001
    ...
    matches any of its values::
        ...
        em_database.filter(technique="4D-STEM")
        em_database.filter(technique="4D-STEM", tags="Strain")
        em_database.filter(microscope_vendor=["JEOL", "Hitachi"])
        em_database.filter(downloaded=True)
        em_database.filter(location="shared")
    ...
    is reached as ``em_database.filter``; nothing here needs the builtin.
    """
